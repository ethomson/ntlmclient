...
#define spec_is_option_type(x) \
	((x)->type == NTLM_OPT_TYPE_BOOL || \
	 (x)->type == NTLM_OPT_TYPE_SWITCH || \
	 (x)->type == NTLM_OPT_TYPE_VALUE)

INLINE(const ntlm_opt_spec *) spec_for_long(
	int *is_negated,
	int *has_value,
	const char **value,
	const ntlm_opt_parser *parser,
	const char *arg)
{
	...
	for (spec = parser->specs; spec->type; ++spec) {
		...
		if (spec->type == NTLM_OPT_TYPE_LITERAL && arg[0] == '\0')
			return spec;

		/* Handle --no-option arguments for bool types */
		if (spec->type == NTLM_OPT_TYPE_BOOL &&
		    strncmp(arg, "no-", 3) == 0 &&
...
INLINE(const ntlm_opt_spec *) spec_for_short(
	const char **value,
	const ntlm_opt_parser *parser,
	const char *arg)
{
...
INLINE(const ntlm_opt_spec *) spec_for_arg(ntlm_opt_parser *parser)
{
	const ntlm_opt_spec *spec;
	size_t args = 0;

	for (spec = parser->specs; spec->type; ++spec) {
		if (spec->type == NTLM_OPT_TYPE_ARG) {
		...
		if (spec->type == NTLM_OPT_TYPE_ARGS && args == parser->arg_idx)
			return spec;
	}
...
INLINE(int) spec_is_choice(const ntlm_opt_spec *spec)
{
	return ((spec + 1)->type &&
	       ((spec + 1)->usage & NTLM_OPT_USAGE_CHOICE));
}
 ...
 */
INLINE(void) consume_choices(const ntlm_opt_spec *spec, ntlm_opt_parser *parser)
{
...
static ntlm_opt_status_t parse_long(ntlm_opt *opt, ntlm_opt_parser *parser)
{
	...
	if (spec->type == NTLM_OPT_TYPE_LITERAL)
		parser->in_literal = 1;

	/* --bool or --no-bool */
	else if (spec->type == NTLM_OPT_TYPE_BOOL && spec->value)
		*((int *)spec->value) = !is_negated;

	/* --accumulate */
	else if (spec->type == NTLM_OPT_TYPE_ACCUMULATOR && spec->value)
		*((int *)spec->value) += spec->switch_value ? spec->switch_value : 1;

	/* --switch */
	else if (spec->type == NTLM_OPT_TYPE_SWITCH && spec->value)
		*((int *)spec->value) = spec->switch_value;

	/* Parse values as "--foo=bar" or "--foo bar" */
	else if (spec->type == NTLM_OPT_TYPE_VALUE) {
		if (has_value)
	...
	if (spec->type == NTLM_OPT_TYPE_VALUE &&
	    !opt->value &&
	    !(spec->usage & NTLM_OPT_USAGE_VALUE_OPTIONAL))
...
static ntlm_opt_status_t parse_short(ntlm_opt *opt, ntlm_opt_parser *parser)
{
	...
	if (spec->type == NTLM_OPT_TYPE_BOOL && spec->value)
		*((int *)spec->value) = 1;
	 ...
	 * the parser index.
	 */
	if (spec->type != NTLM_OPT_TYPE_VALUE && arg[2 + parser->in_short] != '\0') {
		parser->in_short++;
		parser->idx--;
	} else {
		parser->in_short = 0;
	...
	/* Required argument was not provided */
	if (spec->type == NTLM_OPT_TYPE_VALUE && !opt->value)
		opt->status = NTLM_OPT_STATUS_MISSING_VALUE;
	else
		opt->status = NTLM_OPT_STATUS_OK;
...
{
	const ntlm_opt_spec *spec = spec_for_arg(parser);
	...
	if (!spec) {
		parser->idx++;
		opt->status = NTLM_OPT_STATUS_UNKNOWN_OPTION;
	} else if (spec->type == NTLM_OPT_TYPE_ARGS) {
		if (spec->value)
...
{
	if ((flags & NTLM_OPT_PARSE_FORCE_GNU) != 0)
		return 1;

	if ((flags & NTLM_OPT_PARSE_GNU) == 0)
		return 0;
...
INLINE(const ntlm_opt_spec *) spec_for_sort(
	int *needs_value,
	const ntlm_opt_parser *parser,
	const char *arg)
{
 ...
 */
static int sort_gnu_style(ntlm_opt_parser *parser)
{
	...
	for (i = parser->idx; i < parser->args_len; i++) {
		 ...
		 */
		if (spec->type == NTLM_OPT_TYPE_VALUE && needs_value) {
			/*
...
ntlm_opt_status_t ntlm_opt_parser_next(ntlm_opt *opt, ntlm_opt_parser *parser)
{
	...
	if (parser->needs_sort && sort_gnu_style(parser))
		return ntlm_opt_parser_next(opt, parser);

	return parse_arg(opt, parser);
}

INLINE(int) spec_included(const ntlm_opt_spec **specs, const ntlm_opt_spec *spec)
{
	const ntlm_opt_spec **i;
...
static ntlm_opt_status_t validate_required(
	ntlm_opt *opt,
	const ntlm_opt_spec specs[],
	const ntlm_opt_spec **given_specs)
{
	const ntlm_opt_spec *spec, *required;
	int given;
	...
	for (spec = specs, required = NULL, given = 0; spec->type; ++spec) {
		if (!required && (spec->usage & NTLM_OPT_USAGE_REQUIRED)) {
			required = spec;
			given = 0;
		} else if (!required) {
			continue;
		...
		if (!spec_is_choice(spec)) {
			if (!given) {
				opt->spec = required;
				opt->status = NTLM_OPT_STATUS_MISSING_ARGUMENT;
				break;
			}
	...
	unsigned int flags)
{
	...
	given_specs = alloca(sizeof(const ntlm_opt_spec *) * (args_len + 1));

	while (ntlm_opt_parser_next(opt, &parser)) {
		if (opt->status != NTLM_OPT_STATUS_OK &&
		    opt->status != NTLM_OPT_STATUS_DONE)
			return opt->status;

		if ((opt->spec->usage & NTLM_OPT_USAGE_STOP_PARSING))
			return (opt->status = NTLM_OPT_STATUS_DONE);
	...
	const ntlm_opt_spec specs[])
{
	...
	for (spec = specs; spec->type; ++spec) {
		if (!choice)
			optional = !(spec->usage & NTLM_OPT_USAGE_REQUIRED);
		...
		if (spec->type == NTLM_OPT_TYPE_VALUE && spec->alias &&
		    !(spec->usage & NTLM_OPT_USAGE_VALUE_OPTIONAL) &&
		    !(spec->usage & NTLM_OPT_USAGE_SHOW_LONG))
			error = fprintf(file, "-%c <%s>", spec->alias, spec->value_name);
		else if (spec->type == NTLM_OPT_TYPE_VALUE && spec->alias &&
		         !(spec->usage & NTLM_OPT_USAGE_SHOW_LONG))
			error = fprintf(file, "-%c [<%s>]", spec->alias, spec->value_name);
		else if (spec->type == NTLM_OPT_TYPE_VALUE &&
		         !(spec->usage & NTLM_OPT_USAGE_VALUE_OPTIONAL))
			...
			error = fprintf(file, "--%s=<%s>", spec->name, spec->value_name);
		else if (spec->type == NTLM_OPT_TYPE_ARG)
			error = fprintf(file, "<%s>", spec->value_name);
		else if (spec->type == NTLM_OPT_TYPE_ARGS)
			error = fprintf(file, "<%s>...", spec->value_name);
		else if (spec->type == NTLM_OPT_TYPE_LITERAL)
			error = fprintf(file, "--");
		else if (spec->alias && !(spec->usage & NTLM_OPT_USAGE_SHOW_LONG))
			error = fprintf(file, "-%c", spec->alias);
		else
